...
function arrayCombos() {
  var tempcombo = [];
  var combos = [];
  singles.forEach(letter => {
    singles.forEach(letter2 => {
      let tempcombo = [];
      tempcombo.push(letter);
      tempcombo.push(letter2);
...
function navigateRushHour(timeOfDay) {
  var message =
    (timeOfDay >= 600 && timeOfDay <= 900) || (timeOfDay >= 1600 && timeOfDay <= 1800)
      ? `The time of day is ${timeOfDay}, I'm going around the city.`
      : `The time of day is ${timeOfDay}, I'm going through the city.`;
  return message;
  // YOUR CODE HERE
...
let factorial = function(num) {
  var numArray = [];
  //fill array based on num variable provided
  for (var i = 0; i <= num; i++) {
    numArray.push(i);
  }
  //remove array 0 because 0 * anything is 0 = BAD
  numArray.shift();
  //create the value we're going to return
  var fact = 1;
  //multiply each number in the array continually
  for (var j = 0; j < numArray.length; j++) {
    fact = fact * numArray[j];
  }
